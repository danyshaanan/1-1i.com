...
for (yIndex, yVal) in [(v,1.*v/half) for v in range(-half,half+1)]:
  for (xIndex, xVal) in [(v,1.*v/half) for v in range(-half,half+1)]:
    ...
    filename = "julia_" + str(10000 + (yIndex+half)*100 + (xIndex+half)) + ".png"
    image.save(filename)
    print filename
